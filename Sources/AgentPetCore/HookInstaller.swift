...
/// Installs/removes AgentPet's hook entries in an agent's `settings.json`
/// (Claude Code, Codex, and Gemini CLI share the same `{"hooks": {...}}` shape).
///
...
/// install is idempotent and foreign hooks are never touched. The event list is
/// supplied per agent by `AgentHooks`; the constant below is the Claude default.
